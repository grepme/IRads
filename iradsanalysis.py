...
class IradsAnalysis(object):
    ...
    def generate(self, start=None, end=None, patient=None, testType=None):
        ...
        query = session.query(RadiologyRecord, func.count(PacsImage.record_id).label('total')).join(PacsImage).group_by(RadiologyRecord.patient_id, RadiologyRecord.test_type).order_by(RadiologyRecord.patient_id)
