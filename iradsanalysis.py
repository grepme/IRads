...
class IradsAnalysis(object):
    ...
    def generate(self, start=None, end=None, patient=None, testType=None):
        ...
        query = session.query(RadiologyRecord, func.count(RadiologyRecord.test_type, PacsImage.record_id).label('total')).join(PacsImage).group_by(RadiologyRecord.patient_id).order_by('total DESC')
